...
export class WeatherService {
  ...
  /**
   * Busca dados meteorológicos diários da Estação Embrapa Trigo
   * Fonte: Laboratório de Agrometeorologia / Embrapa Trigo via /api/weather/embrapa
   * @returns {Promise<Object>} Dados estruturados da estação Embrapa Trigo
   */
  static async fetchEmbrapaDailyData() {
    const cacheKey = 'embrapa_trigo_daily_v1';
    const now = Date.now();
    const CACHE_TTL_MS = 15 * 60 * 1000; // 15 minutos

    // 1. Verificar se existe cache fresco válido
    try {
      const cached = localStorage.getItem(cacheKey);
      if (cached) {
        const parsed = JSON.parse(cached);
        if (parsed.timestamp && (now - parsed.timestamp < CACHE_TTL_MS) && parsed.data && parsed.data.success) {
          return parsed.data;
        }
      }
    } catch (e) {
    ...
    // 2. Chamar proxy Serverless seguro (/api/weather/embrapa)
    try {
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 8000);
      const res = await fetch('/api/weather/embrapa', {
      ...
      console.warn('[WeatherService] Falha ao consultar /api/weather/embrapa:', err.message);
    }

    // 3. Fallback: usar cache persistido se a rede falhar
    try {
      const cached = localStorage.getItem(cacheKey);
      if (cached) {
        const parsed = JSON.parse(cached);
        if (parsed.data) {
          console.warn('[WeatherService] Usando dados em cache de Embrapa Trigo.');
          return parsed.data;
        }
      }
    } catch (e) {}
    ...
    return {
      success: false,
      error: 'Não foi possível carregar os dados da Estação Embrapa Trigo no momento.'
    };
  }

  /**
   * Recupera dados da Embrapa Trigo do cache local síncrono (para popups)
   * @returns {Object|null}
   */
  static getCachedEmbrapaDailyData() {
    try {
      const stored = localStorage.getItem('embrapa_trigo_daily_v1');
      if (stored) {
        const parsed = JSON.parse(stored);
